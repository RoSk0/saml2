...
final class BindingTest extends TestCase
{
    ...
    public function testBindingGuesserPOST(): void
    {
        ...
        $q = ['SAMLart' => 'AAQAAI4sWYpfoDDYJrHzsMnG+jyNM94p5ejn49a+nZ0s3ylY7knQ6tkLMDE='];
        $request = new ServerRequest('POST', 'http://tnyholm.se');
        $request = $request->withParsedBody($q);
        $bind = Binding::getCurrentBinding($request);
        $this->assertInstanceOf(HTTPArtifact::class, $bind);
